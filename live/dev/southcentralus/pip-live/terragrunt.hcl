...
inputs = {
  name                = "pip-aks-${local.environment}-${local.location_short}-live"
  resource_group_name = dependency.network.outputs.resource_group_name
  location            = local.location
  domain_name_label   = "live-aks-${local.environment}-${local.location_short}"
  tags = merge(local.common_tags, { Component = "Public IP", Slot = "live" })
}
