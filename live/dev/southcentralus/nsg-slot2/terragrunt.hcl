...
dependency "network" {
  ...
  mock_outputs = {
    ...
    subnet_ids = {
      "snet-aks-slot2-system" = "/subscriptions/mock/subnets/mock"
      "snet-aks-slot2-apps"   = "/subscriptions/mock/subnets/mock"
    }
  }
...
inputs = {
  name                = "nsg-aks-${local.environment}-${local.location_short}-slot2"
  resource_group_name = dependency.network.outputs.resource_group_name
  location            = local.location
  allowed_ip_ranges   = [local.env_vars.locals.deployment_ip]
  subnet_ids = {
    system = dependency.network.outputs.subnet_ids["snet-aks-slot2-system"]
    apps   = dependency.network.outputs.subnet_ids["snet-aks-slot2-apps"]
  }
  tags = merge(local.common_tags, { Component = "NSG", Slot = "slot2" })
}
