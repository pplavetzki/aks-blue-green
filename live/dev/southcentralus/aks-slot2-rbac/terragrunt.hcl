...
inputs = {
  role_assignments = {
    ...
    network_contributor_cluster = {
      scope                = "/subscriptions/${local.env_vars.locals.subscription_id}"
      role_definition_name = "Network Contributor"
      principal_id         = dependency.identity.outputs.principal_id
      description          = "Allow AKS slot1 cluster identity to manage network resources"
    }
  }
}